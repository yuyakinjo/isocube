...
describe("generator", () => {
  ...
  it("wraps at cumulative offsets and cycles normalized RGB colors across rows", () => {
    ...
    assert.deepEqual(generateLogo({ text: "ABCDEFGHI", breakAt: [3, 6] }).lines, [
      "ABC",
      "DEF",
      "GHI",
    ]);
  ...
  it("reproduces random output when the returned colors are reused", () => {
    ...
    assert.equal(generateLogo({ text: "DECOPIN", colors: first.colors }).svg, first.svg);
  });
  it("rejects invalid text, wraps, and colors", () => {
    ...
    for (const colors of [[], ["red"], ["#ffff"], ["rgb(256,0,0)"], ["rgb(-1,0,0)"]]) {
