...
export function releaseNotes(commits: readonly Commit[]): string {
  const breaking = commits.flatMap(({ subject, body }) => {
    const description =
      /^BREAKING[ -]CHANGE:\s*(.+)$/im.exec(body)?.[1] ??
      /^\w+(?:\([^)]*\))?!:\s*(.+)$/.exec(subject)?.[1];
  ...
  sections.push(
    ['## Changes', '', ...commits.map(({ subject }) => `- ${subject}`)].join(
      '\n'
    )
  );
...
if (import.meta.main) {
  ...
  const range =
    described.status === 0 ? `${described.stdout.trim()}..HEAD` : 'HEAD';
  const log = execFileSync('git', ['log', range, '--format=%B%x00'], {
    encoding: 'utf8',
  });
